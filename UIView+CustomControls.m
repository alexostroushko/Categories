...
{
    [self showInTaxiActivityViewWithYOffset:0];
}

- (void)showInTaxiActivityViewWithYOffset:(float)offset
{
    [self showInTaxiActivityViewWithYOffset:offset andBackgroundColor:[UIColor clearColor]];
}

- (void)showInTaxiActivityViewWithYOffset:(float)offset andBackgroundColor:(UIColor*)color
{
    CGRect frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y + offset, self.bounds.size.width, self.bounds.size.height);
    ...
    CGPoint center = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2 + offset);
    INPopupViewActivity *activityIndicator = [INPopupViewActivity viewWithCenter:center];
    [activityIndicator initPopupView];
    [activityIndicator setHidden:NO withAnimation:YES];
