...
- (BOOL)isEqualToDateIgnoringMinutes:(NSDate *)aDate
{
    NSDateComponents *components1 = [CURRENT_CALENDAR components:DATE_COMPONENTS fromDate:self];
	NSDateComponents *components2 = [CURRENT_CALENDAR components:DATE_COMPONENTS fromDate:aDate];
	return ((components1.year == components2.year) &&
...
#pragma mark - Compare between 2 custom dates

- (BOOL) isEqualIgnoringToCustomDate:(NSDate *)date
{
    NSCalendar *calendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar];
    [calendar setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
    
	NSDateComponents *components1 = [calendar components:DATE_COMPONENTS fromDate:self];
	NSDateComponents *components2 = [calendar components:DATE_COMPONENTS fromDate:date];
	return ((components1.year == components2.year) &&
			(components1.month == components2.month) &&
			(components1.day == components2.day));
}
...
- (NSDate *)dateTomorrowForDate:(NSDate *)date
{
    return [self dateWithDays:1 fromDate:date];
}

- (BOOL)isTodayForDate:(NSDate *)date
{
    return [date isEqualIgnoringToCustomDate:self];
}

- (BOOL)isTomorrowForDate:(NSDate *)date
{
    return [self isEqualIgnoringToCustomDate:[date dateTomorrowForDate:date]];
}
